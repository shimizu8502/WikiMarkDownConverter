# PukiWiki to Markdown Converter v20250613_0957
...
**🆕 最新アップデート (v20250613_0957)**
- **🗑️ アンカー削除機能**: `[#文字列]` 形式のアンカーパターンを自動削除
- **🧹 クリーンアップ機能**: 不要なアンカータグを除去してMarkdown出力をより見やすく
- **📝 リスト項目インデント機能**: リスト項目の後続行に自動的にタブインデントを追加 
- **🔧 複数行対応**: リスト項目に続く複数行の説明文すべてにインデントを適用
- **🎯 視覚的改善**: より読みやすいMarkdown出力を実現
...
- `&color(red,yellow){テキスト}` → `<span style="color: red; background-color: yellow;">テキスト</span>`
- `&color(,yellow){テキスト}` → `<span style="background-color: yellow;">テキスト</span>`
- `&color(red,){テキスト}` → `<span style="color: red;">テキスト</span>`
...
#### リスト項目インデント機能 ⭐NEW v20250612⭐
リスト項目の後続行に自動的にタブインデントを追加し、視覚的に読みやすいMarkdown形式を生成します。

**変換前:**
```
- 次世代タイヤを使用してみた(2024/12)
ドライ、ウェットはサマータイヤと違いは感じない（乗り心地、制動距離）
2行目の文字列
3行目の文字列
- その他
スタッドレス：ドライ、特にウェット路面に弱い
```
...
**特徴:**
- **複数行対応**: リスト項目に続く複数行の説明文すべてにインデント適用
- **自動判定**: リスト項目、見出し、テーブル行、コードブロックは除外
- **既存インデント保護**: 既にインデントされている行は重複処理を回避
- **視覚的改善**: Obsidianでの表示がより見やすくなる
...
#### 画像
- `#ref(画像URL)` → `![](画像URL)`
- `#ref(画像URL,altテキスト)` → `![altテキスト](画像URL)`
...
#### 文字揃え指定
- `CENTER:テキスト` または `C:テキスト` → `:---:` (中央揃え)
- `RIGHT:テキスト` または `R:テキスト` → `---:` (右揃え)
- `LEFT:テキスト` または `L:テキスト` → `:---` (左揃え)
...
- `// コメント` → (削除)
- `text // コメント` → `text`
- `https://example.com` → `https://example.com` (URLは保護)

#### アンカー削除 ⭐NEW v20250613⭐
PukiWiki の `[#文字列]` 形式のアンカーパターンを自動的に削除します。

**変換例:**
```
# 見出し1 [#heading1]         → # 見出し1
テキスト [#anchor123] 続き      → テキスト  続き
内容 [#test] の説明            → 内容  の説明
```

**特徴:**
- **完全除去**: `[#文字列]` パターンを完全に削除
- **柔軟対応**: 任意の文字列（日本語、英数字、記号）に対応
- **処理順序**: 見出しリンク処理（`[[#文字列]]`）の後に実行されるため、既存機能に影響なし
- **視覚的改善**: 不要なアンカータグを除去してより見やすいMarkdown出力を実現

### 📁 ファイル処理機能

#### ファイル名処理
- **16進エンコードファイル名**: 自動UTF-8デコード
...
#### 文字コード対応
- **自動判別**: UTF-8, EUC-JP, Shift_JIS
- **手動指定**: utf-8, euc-jp, shift_jis

### 🎨 対応色形式 (色指定機能)

#### CSS色名
- `red`, `blue`, `green`, `yellow`, `black`, `white`, `navy`, `gray`, etc.

#### 16進数カラーコード
- `#ff0000` (赤), `#00ff00` (緑), `#0000ff` (青), `#ffffff` (白), `#000000` (黒)

#### RGB値・その他CSS色指定
- `rgb(255,0,0)`, `rgba(255,0,0,0.5)`, `hsl(0,100%,50%)`
...
#### 処理上の制限
- 文字コード自動判別は100%正確ではない
- 非常に大きなファイルでは処理時間がかかる場合がある
- 一部の特殊文字が正しく変換されない場合がある
...
### v20250613_0957 🆕
- **アンカー削除機能**: `[#文字列]` 形式のアンカーパターンを自動削除
- **クリーンアップ処理**: 不要なアンカータグを除去してMarkdown出力をより見やすく
- **後処理最適化**: 見出しリンク処理後にアンカー削除を実行して最適な順序で処理

### v20250612_1641
- **リスト項目インデント機能**: リスト項目の後続行に自動的にタブインデントを追加
- **複数行対応**: リスト項目に続く複数行の説明文すべてにインデント適用
- **自動判定機能**: リスト項目、見出し、テーブル行、コードブロックを自動判別して適切に処理
- **既存インデント保護**: 既にインデントされている行の重複処理を回避
- **視覚的改善**: Obsidianでの表示がより見やすく、構造化された出力を実現

### 過去のバージョン
- **v20250606_1524**: 装飾記法（フォントサイズ・色指定）対応、自動更新機能追加
- **v1.4**: 更新変換モード、タイムスタンプ管理機能
- **v1.3**: 全変換モード、設定管理機能
- **v1.2**: GUI改善、エラー処理強化
- **v1.1**: 表組み対応、ファイル名デコード機能
- **v1.0**: 基本的なPukiWikiからMarkdownへの変換機能
